export const API_URL= `${process.env.API_URL ?? ""}`;
export const WEB_URL= "http://localhost:3000"

export const POAP_BASE_URL = "https://api.thegraph.com/subgraphs/name/poap-xyz/poap-xdai";
export const OPENSEA_BASE_URL = "https://api.opensea.io/api/v1";
export const SNAPSHOT_BASE_URL = "https://hub.snapshot.org/graphql";

export const ETHPLORER_BASE_URL = "https://api.ethplorer.io";
export const ARWEAVE_BASE_URL = "https://arweave.net/graphql";

export const API_BASE_PATH = `${process.env.BASE_URL ?? ""}/api`;
export const API_PINS_PATH = `${API_BASE_PATH}/pin`;
export const API_WALLET_PATH = `${API_BASE_PATH}/wallet`;
export const API_WALLET_PATH2 = `${API_BASE_PATH}/login`;


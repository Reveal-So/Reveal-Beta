...
const getFeedsNFTs = async ({
  ...
  try {
    ...
    let consulta = address
      ? `${API_URL}/api/Feed/GetNfts?page=${page}&walletAddress=${address}`
      : `${API_URL}/api/Feed/GetNfts`;
...
const getFeedsDAOs = async (
  ...
  try {
    ...
    //DAOs
    let consulta = address
      ? `${API_URL}/api/Feed/GetDaos?page=${page}&walletAddress=${address}`
      : `${API_URL}/api/Feed/GetDaos`;
    result = await fetch(consulta, {
      method: "GET",
    });
    data = await result.json();
...
const getFeedsTOKENs = async (
  ...
  try {
    ...
    //TOKENs
    let consulta = address
      ? `${API_URL}/api/Feed/GetTokens?page=${page}&walletAddress=${address}`
      : `${API_URL}/api/Feed/GetTokens`;
    result = await fetch(consulta, {
      method: "GET",
    });
    data = await result.json();
...
const getFeedsPOAPs = async (
  ...
  try {
    ...
    //Poaps
    let consulta = address
      ? `${API_URL}/api/Feed/GetPoaps?page=${page}&walletAddress=${address}`
      : `${API_URL}/api/Feed/GetPoaps`;
    result = await fetch(consulta, {
      method: "GET",
    });
    data = await result.json();
...
const getFeedsMIRRORs = async (
  ...
  try {
    ...
    //Mirrors
    let consulta = address
      ? `${API_URL}/api/Feed/GetMirrors?page=${page}&walletAddress=${address}`
      : `${API_URL}/api/Feed/GetMirrors`;
...
export const FeedScreen = () => {
  const { address, web3Provider ,followingCount } = useContext(UserContext);
 ////////////////////////////////////////////////////////////////////////////////////
//  const { loading, Feeds, setPage, loadingNextPage,errorNft,errorDaos,errorTokens,errorMirrors,errorPoaps } = useGetFeeds({
//   address: address,
// });
...
const [loading, setLoading] = useState(false);
 ...
 ////////////////////////////////////////////////////////////////////////////////////
  ...
  // useEffect(() => {
  //   console.log("update Feeds",Feeds);
  // }, [Feeds]);
    ...
    <>
      <div className=" bg-white ">
        ...
        <div className=" sm:flex w-full  ">
                ...
                <nav
                  ...
                  <div className="space-y-1">
                    <div
                      ...
                      {/* item.name == nameCat
                              ? " bg-[#CABDE8] text-white"
                              : " text-[#6F41D8] ",
                            " group flex items-center pl-10 px-3 py-2 font-medium  rounded-md w-full",
                            " hover:text-white hover:bg-[#CABDE8] ", 
                      ...
                      {PrimaryNavigation.map((item) => (
                        <button
                          ...
                          className={classNames(
                            ...
                            "flex items-center px-3 py-1 text-sm font-medium rounded-md w-full "
                          )}>
          ...
          {/* aqui va el listado que llega flex justify-center items-center  sm:pl-[250px]*/}
          {/*  */}
          <div className="relative w-full h-full px-2 sm:pl-[20%] lg:pl-0 sm:px-0  ">
            ...
            {/* loading:{`${loading}`} - bandCargando:{`${bandCargando}`}- bandError:{`${bandError}`}- Feeds:{`${Feeds.length}`} */}
              {followingCount==0?<>You do not follow anyone yet</>: 
              loading? (
            ...
            <div className="flex absolute w-full h-[35%] inset-y-0 top-[65%] left-0 items-center  pointer-events-none px-5  ">
              <div
                id="visor"
                ref={externalRef as LegacyRef<HTMLDivElement>}
                className={"h-[full] w-[100px]  "}></div>
            </div>
          </div>
        </div>
      </div>

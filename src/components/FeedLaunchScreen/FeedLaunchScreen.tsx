...
const getFeedsNFTs = async (
  ...
  try {
    ...
    let consulta = address
      ? `${API_URL}/api/Feed/GetNfts?page=${page}&walletAddress=${address}&IsLaunchHouse=true`
      : `${API_URL}/api/Feed/GetNfts?page=${page}&IsLaunchHouse=true`;
...
const getFeedsDAOs = async (
  ...
  try {
    ...
    //DAOs
    let consulta = address
      ? `${API_URL}/api/Feed/GetDaos?page=${page}&walletAddress=${address}&IsLaunchHouse=true`
      : `${API_URL}/api/Feed/GetDaos?page=${page}&IsLaunchHouse=true`;
    result = await fetch(consulta, {
      method: "GET",
    });
    data = await result.json();
...
const getFeedsTOKENs = async (
  ...
  try {
    ...
    //TOKENs
    let consulta = address
      ? `${API_URL}/api/Feed/GetTokens?page=${page}&walletAddress=${address}&IsLaunchHouse=true`
      : `${API_URL}/api/Feed/GetTokens?page=${page}&IsLaunchHouse=true`;
    result = await fetch(consulta, {
      method: "GET",
    });
    data = await result.json();
...
const getFeedsPOAPs = async (
  ...
  try {
    ...
    //Poaps
    let consulta = address
      ? `${API_URL}/api/Feed/GetPoaps?page=${page}&walletAddress=${address}&IsLaunchHouse=true`
      : `${API_URL}/api/Feed/GetPoaps?page=${page}&IsLaunchHouse=true`;
    result = await fetch(consulta, {
      method: "GET",
    });
    data = await result.json();
...
const getFeedsMIRRORs = async (
  ...
  try {
    ...
    //Mirrors
    let consulta = address
      ? `${API_URL}/api/Feed/GetMirrors?page=${page}&walletAddress=${address}&IsLaunchHouse=true`
      : `${API_URL}/api/Feed/GetMirrors?page=${page}&IsLaunchHouse=true`;
    result = await fetch(consulta, {
      method: "GET",
    });
    data = await result.json();
...
const useGetFeeds: any = ({ address } = { address: null }) => {
  ...
  useEffect(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [address, page,setErrorDaos,setErrorMirrors,setErrorPoaps,setErrorNft,setErrorTokens,getFeedsTOKENs,getFeedsDAOs,getFeedsMIRRORs,getFeedsPOAPs,getFeedsNFTs]
  );
  return { loading, loadingNextPage, error, Feeds, setPage, setLoading, errorNft,errorDaos,errorTokens,errorMirrors,errorPoaps,cargaNft,cargaDaos,cargaTokens,cargaMirrors,cargaPoaps,loadingNextNft,loadingNextDaos,loadingNextTokens,loadingNextMirrors,loadingNextPoaps };
};
////////////////////////////////////////////////////////////////////////////////////
export const FeedLaunchScreen: FC = () => {
  const { address, web3Provider } = useContext(UserContext);
  const { loading, Feeds, setPage, loadingNextPage,errorNft,errorDaos,errorTokens,errorMirrors,errorPoaps,cargaNft,cargaDaos,cargaTokens,cargaMirrors,cargaPoaps,loadingNextNft,loadingNextDaos,loadingNextTokens,loadingNextMirrors,loadingNextPoaps } = useGetFeeds({
    address: null,
  });
  const [nameCat, setnameCat] = useState("All");
  const debounceHandleNextPage = useCallback(
    debounce(() => {
      if (!loadingNextPage) setPage((prevPage: any) => prevPage + 1);
  ...
  return (
    <>
      ...
      {address?<TabsFeed href={"/feedlaunch"} />:<TabsFeed href={"/feedlaunch"} notAddress={true} />}
        <div className=" sm:flex w-full  ">
        {/* md:w-[160px] */}
        <div className="sm:fixed w-full z-30 sm:w-[100px]  lg:w-[150px] sm:pt-[100px] left-0 sm:left-[1%] md:left-[5%] lg:left-[10%]  ">
          ...
          {/* aqui va el listado que llega flex justify-center items-center  */}
          <div className="relative w-full h-full px-2 sm:pl-[20%] lg:pl-0 sm:px-0  ">
          {/*w-full  lg:pl-[30%] xl:pl-[25%] 2xl:pl-[28%] */}
          <div className=" lg:mx-auto max-w-[500px]  pt-3 sm:pt-[130px] space-y-4  ">
            {loading? (

...
export const resolveMirrorProfile = async (address: string): Promise<any[]|null> => {
  const query = `
...
export const useMirrorProfile = () => {
  ...
  const [mirrorProfile, setMirrorProfile] = useState<any>();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setMirrorProfile(await resolveMirrorProfile(profileAddress.toString()));
    };
